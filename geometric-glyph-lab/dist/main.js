import { ALL_FAMILIES, CELL_HEIGHT, CELL_WIDTH, FAMILY_DEFINITIONS, bitmapAscii, exportAtlasPng, exportJson, formatPort, generate, hasPixel, } from "./core/index.js";
...
`;
const elements = {
    familyList: requireElement("family-list"),
    selectAll: requireElement("select-all"),
    generate: requireElement("generate"),
    candidateCount: requireElement("candidate-count"),
    uniqueCount: requireElement("unique-count"),
    duplicateCount: requireElement("duplicate-count"),
    compressionCount: requireElement("compression-count"),
    exportJson: requireElement("export-json"),
    exportPng: requireElement("export-png"),
    glyphTitle: requireElement("glyph-title"),
    codepointChip: requireElement("codepoint-chip"),
    glyphPreview: requireElement("glyph-preview"),
    actualPreview: requireElement("actual-preview"),
    glyphId: requireElement("glyph-id"),
    bitmapKey: requireElement("bitmap-key"),
    aliasCount: requireElement("alias-count"),
    familyCount: requireElement("family-count"),
    aliasSummary: requireElement("alias-summary"),
    aliasList: requireElement("alias-list"),
    asciiPreview: requireElement("ascii-preview"),
    glyphSearch: requireElement("glyph-search"),
    atlasStatus: requireElement("atlas-status"),
    atlasScroll: requireElement("atlas-scroll"),
    atlasCanvas: requireElement("atlas-canvas"),
    atlasEmpty: requireElement("atlas-empty"),
};
const selectedFamilies = new Set(ALL_FAMILIES);
let result = null;
...
function syncFamilyControls() {
    const checkboxes = elements.familyList.querySelectorAll("input[data-family]");
...
function drawAtlasGlyph(context, glyph, index, selected) {
    ...
    for (let y = 0; y < CELL_HEIGHT; y += 1) {
        for (let x = 0; x < CELL_WIDTH; x += 1) {
            if (hasPixel(glyph.bitmap, x, y)) {
                context.fillRect(originX + x * ATLAS_SCALE, originY + y * ATLAS_SCALE, ATLAS_SCALE, ATLAS_SCALE);
            }
        }
    }
    if (selected) {
...
function selectGlyph(index, scrollIntoView) {
...
function scrollAtlasToIndex(index) {
    const row = Math.floor(index / ATLAS_COLUMNS);
...
function renderInspector(glyph) {
...
function drawBitmap(canvas, glyph, scale, drawGrid) {
    ...
    for (let y = 0; y < CELL_HEIGHT; y += 1) {
        for (let x = 0; x < CELL_WIDTH; x += 1) {
            if (hasPixel(glyph.bitmap, x, y)) {
                ...
                context.fillRect(x * scale + inset, y * scale + inset, Math.max(1, scale - inset), Math.max(1, scale - inset));
            }
        }
    }
}
