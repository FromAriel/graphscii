...
export function setPixel(bitmap, x, y) {
    if (x < 0 || x >= CELL_WIDTH || y < 0 || y >= CELL_HEIGHT) {
        return;
    }
    bitmap[y] |= 1 << x;
}
export function hasPixel(bitmap, x, y) {
    return (bitmap[y] & (1 << x)) !== 0;
}
export function rasterizeLineBetweenPixels(start, end) {
