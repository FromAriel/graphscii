...
export const ALL_FAMILIES = new Set<FamilyId>(FAMILY_DEFINITIONS.map((family) => family.id));

export function generate(selectedFamilies: ReadonlySet<FamilyId>): GenerationResult {
  const candidates: CandidateGlyph[] = [];
  ...
  const byBitmap = new Map<string, UniqueGlyph>();
  const glyphs: UniqueGlyph[] = [];
