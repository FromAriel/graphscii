import {
  ...
  hasPixel,
  rasterizeStraightStyledFill,
  type CandidateGlyph,
  type StraightFillStyle,
  type StraightStyledFillCandidate,
} from "./core/index.js";
...
function drawResearchAtlas(
  canvas: HTMLCanvasElement,
  style: StraightFillStyle,
  side: "A" | "B",
): void {
  const candidates = fillResearch.candidates.filter(
    (candidate) => candidate.style === style && candidate.side === side,
  );
  const columns = 32;
...
function ownerText(fill: StraightStyledFillCandidate): { text: string; reuse: boolean } {
...
function mountStraightFillExplorer(): void {
  ...
  const styleSelect = panel.querySelector<HTMLSelectElement>("#fill-style-select");
  const atlasSideSelect = panel.querySelector<HTMLSelectElement>("#fill-atlas-side-select");
  ...
  const atlasCanvas = panel.querySelector<HTMLCanvasElement>("#fill-dither-atlas-canvas");
  ...
  const atlasOwner = panel.querySelector<HTMLElement>("#fill-atlas-owner");
  const styleResult = panel.querySelector<HTMLElement>("#fill-style-result");
  ...
  const render = (): void => {
    ...
    const style = styleSelect.value as StraightFillStyle;
    const atlasSide = atlasSideSelect.value as "A" | "B";
