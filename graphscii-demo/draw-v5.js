(() => {
  ...
  function makePort(cellX, cellY, side, index) {
    ...
    const count = side === 'L' || side === 'R' ? SIDE_NODES : TOP_BOTTOM_NODES;
    if (!['L', 'R', 'T', 'B'].includes(side) || index < 0 || index >= count) {
  ...
  function portPoint(port) {
    if (port.side === 'L' || port.side === 'R') {
      return {
        ...
        y: port.cellY * CELL_H + (port.index / (SIDE_NODES - 1)) * CELL_H,
      };
    }
    return {
      x: port.cellX * CELL_W + (port.index / (TOP_BOTTOM_NODES - 1)) * CELL_W,
  ...
  function canonicalNodeKey(port) {
    if (port.side === 'L' || port.side === 'R') {
      ...
      if (port.index === 0) return `K:${boundaryX}:${port.cellY}`;
      if (port.index === SIDE_NODES - 1) return `K:${boundaryX}:${port.cellY + 1}`;
      return `V:${boundaryX}:${port.cellY}:${port.index}`;
    }

    const boundaryY = port.cellY + (port.side === 'B' ? 1 : 0);
    if (port.index === 0) return `K:${port.cellX}:${boundaryY}`;
    if (port.index === TOP_BOTTOM_NODES - 1) return `K:${port.cellX + 1}:${boundaryY}`;
  ...
  function snapVerticalIndex(cellY, y) {
    ...
    const spacing = CELL_H / (SIDE_NODES - 1);
    return clamp(Math.round(localY / spacing), 0, SIDE_NODES - 1);
  }

  function snapHorizontalIndex(cellX, x) {
    const localX = clamp(x - cellX * CELL_W, 0, CELL_W);
    const spacing = CELL_W / (TOP_BOTTOM_NODES - 1);
    return clamp(Math.round(localX / spacing), 0, TOP_BOTTOM_NODES - 1);
  }
  ...
  function segmentCrossings(a, b) {
    ...
    out.sort((u, v) => {
      if (Math.abs(u.t - v.t) > 1e-9) return u.t - v.t;
  ...
  function transitionForCrossing(rawCell, crossing, dx, dy) {
    let exit;
    let toCell;
  ...
  function rawTransitions(points) {
    ...
    for (let i = 1; i < points.length; i += 1) {
      ...
      for (const crossing of segmentCrossings(a, b)) {
        const transition = transitionForCrossing(rawCell, crossing, dx, dy);
  ...
  function makeSegment(cell, from, to) {
    ...
    if (canonicalNodeKey(from) === canonicalNodeKey(to)) return null;
  ...
  function compileRawPath(points) {
    ...
    for (const transition of transitions) {
      ...
      if (current.entry.side === transition.exit.side) {
        ...
        segment = makeSegment(current.cell, current.entry, transition.exit);
    ...
    if (current.entry.side !== endPort.side) {
      const tail = makeSegment(current.cell, current.entry, endPort);
  ...
  function lineGuide(a, b) { return [a, b]; }
    ...
    for (let x = 0; x < COLS - 1; x += 1) {
      for (let y = 0; y < ROWS; y += 1) {
        for (let i = 0; i < SIDE_NODES; i += 1) {
          ...
          const b = mate(a);
          if (!b || b.side !== 'L' || b.index !== i || canonicalNodeKey(a) !== canonicalNodeKey(b)) throw new Error('R/L mate invariant failed');
        }
      }
    }
    for (let y = 0; y < ROWS - 1; y += 1) {
      for (let x = 0; x < COLS; x += 1) {
        for (let i = 0; i < TOP_BOTTOM_NODES; i += 1) {
          ...
          const b = mate(a);
          if (!b || b.side !== 'T' || b.index !== i || canonicalNodeKey(a) !== canonicalNodeKey(b)) throw new Error('B/T mate invariant failed');
        }
      }
    }
  }
  ...
  const API = {
    COLS, ROWS, CELL_W, CELL_H, TOP_BOTTOM_NODES, SIDE_NODES,
    STRAIGHT_CODEPOINT_BY_PAIR, makePort, portText, portPoint,
    canonicalNodeKey, mate, cellForPoint, snapPointToCellPerimeter,
    segmentCrossings, compileRawPath, lineGuide, bezierGuide,
    ellipseGuide, validateSegments, selfTest,
  };

  globalThis.GraphSCIIDrawV5 = API;
  ...
  function nearestVisibleNode(point) {
    ...
    for (const side of ['L', 'R', 'T', 'B']) {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        candidates.push({ point: p, d: Math.hypot(p.x - point.x, p.y - point.y) });
      }
    }
  ...
  function drawNodes() {
    ...
    for (let row = 0; row <= ROWS; row += 1) {
      ...
      for (let cellX = 0; cellX < COLS; cellX += 1) {
        for (let i = 0; i < TOP_BOTTOM_NODES; i += 1) {
          const x = cellX * CELL_W + (i / (TOP_BOTTOM_NODES - 1)) * CELL_W;
    ...
    for (let col = 0; col <= COLS; col += 1) {
      ...
      for (let cellY = 0; cellY < ROWS; cellY += 1) {
        for (let i = 1; i < SIDE_NODES - 1; i += 1) {
          const y = cellY * CELL_H + (i / (SIDE_NODES - 1)) * CELL_H;
          overlayCtx.fillRect(x - 1, y - 1, 2, 2);
        }
      }
    }
  ...
  function instruction(tool) {
    if (tool === 'freehand') return 'Freehand: real mouse edge crossings snap once to the node on that exact edge.';
    if (tool === 'line') return 'Line: one straight guide, compiled by the same edge-crossing rules.';
    if (tool === 'bezier') return 'Bezier: click four points; the curve is compiled by the same edge-crossing rules.';
    return 'Ellipse: drag bounds; the ellipse is compiled by the same edge-crossing rules.';
  }
