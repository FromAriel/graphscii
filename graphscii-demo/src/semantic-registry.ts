...
import type { ConnectorFamily } from "./connector-topology";
...
export class GlyphRegistry {
  ...
  private readonly connectorsBySignature = new Map<string, ConnectorRule[]>();
  ...
  private constructor(
    ...
    const addConnector = (family: ConnectorFamily, points: Point[], bitmapKey: string, id: string): void => {
      ...
      const rules = this.connectorsBySignature.get(signature) ?? [];
      if (!rules.some((rule) => rule.family === family && rule.id === id)) rules.push({ family, id, glyph });
      this.connectorsBySignature.set(signature, rules);
    };

    for (const semantic of orthogonal.semantics) {
      addConnector("orthogonal", orthogonalPoints(semantic), semantic.bitmapKey, semantic.id);
    }
    ...
    for (const semantic of diagonal.semantics) {
      ...
      selectedCount += 1;
      addConnector("diagonal", diagonalPoints(semantic), semantic.bitmapKey, semantic.id);
    }
  ...
  resolveConnector(ports: readonly PortName[], family: ConnectorFamily): GraphGlyph | null {
    const signature = pointSignature(ports.map(portPoint));
    if (signature.split("|").filter(Boolean).length < 3) return null;
    const rules = (this.connectorsBySignature.get(signature) ?? []).filter((rule) => rule.family === family);
    const unique = new Map<number, GraphGlyph>();
    for (const rule of rules) unique.set(rule.glyph.codepointValue, rule.glyph);
