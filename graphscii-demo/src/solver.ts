    ...
    for (const tone of TONES) {
      ...
      for (let y = 0; y < height; y += 1) {
        ...
        for (let x = 0; x < width; x += 1) {
          ...
          let alpha = 0;
          const sampleX = x * SUPERSAMPLE;
          const sampleY = y * SUPERSAMPLE;
          for (let sy = 0; sy < SUPERSAMPLE; sy += 1) {
            const rowOffset = (sampleY + sy) * this.canvas.width;
            for (let sx = 0; sx < SUPERSAMPLE; sx += 1) {
              alpha += image[((rowOffset + sampleX + sx) * 4) + 3]!;
            }
          }
          const coverage = alpha / (255 * SUPERSAMPLE * SUPERSAMPLE);
...
function solveCell(
  ...
  return best?.codepointValue ?? BLANK_CODEPOINT;
}
  ...
  solve(objects: DrawingObject[], cellRect: CellRect): void {
    ...
    const extractCoverage = (column: number, row: number): Float32Array => {
      ...
      for (let y = 0; y < CELL_HEIGHT; y += 1) {
        ...
        for (let x = 0; x < CELL_WIDTH; x += 1) cell[y * CELL_WIDTH + x] = target[sourceStart + x]!;
      }
      return cell;
    };
    ...
    // One reverse relaxation pass lets already-solved right/bottom neighbors contribute to continuity.
