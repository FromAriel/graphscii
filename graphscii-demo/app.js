(() => {
  ...
  function buildStraightLookup() {
    ...
    if (Object.keys(byPair).length !== 1664 || nextOwner !== 746) {
      throw new Error('GraphSCII straight lookup invariant failed.');
    }
  ...
  function nearestNode(x, y) {
    ...
    const verticalIndex = clamp(
      Math.round(((y - verticalBand * CELL_H) / CELL_H) * 16 - 0.5),
      0,
      15,
    );
    ...
    const horizontalIndex = clamp(
      Math.round(((x - horizontalBand * CELL_W) / CELL_W) * 8 - 0.5),
      0,
      7,
    );
  ...
  function cellContainsPoint(cell, point, margin = EPS) {
    return (
      point.x >= cell.x * CELL_W - margin &&
      point.x <= (cell.x + 1) * CELL_W + margin &&
      point.y >= cell.y * CELL_H - margin &&
      point.y <= (cell.y + 1) * CELL_H + margin
    );
  }
  ...
  function chooseInitialCell(startNode, nextPoint) {
    const origin = nodePosition(startNode);
    const dx = nextPoint.x - origin.x;
    const dy = nextPoint.y - origin.y;
    const length = Math.hypot(dx, dy) || 1;
    const probe = {
      x: origin.x + (dx / length) * 0.25,
      y: origin.y + (dy / length) * 0.25,
    };

    const adjacent = adjacentCellsForNode(startNode);
    for (const cell of adjacent) {
      if (cellContainsPoint(cell, probe, 0)) return cell;
    }
  ...
  function neighborCell(cell, edge) {
    if (edge === 'L') return { x: cell.x - 1, y: cell.y };
    if (edge === 'R') return { x: cell.x + 1, y: cell.y };
    if (edge === 'T') return { x: cell.x, y: cell.y - 1 };
  ...
  function quantizedPort(cell, edge, point) {
    if (edge === 'L' || edge === 'R') {
      const localY = point.y - cell.y * CELL_H;
      const index = clamp(Math.round((localY / CELL_H) * 16 - 0.5), 0, 15);
      return `${edge}${index}`;
    }

    const localX = point.x - cell.x * CELL_W;
    const index = clamp(Math.round((localX / CELL_W) * 8 - 0.5), 0, 7);
    return `${edge}${index}`;
  }
  ...
  function firstCellExit(cell, start, end) {
    ...
    for (const hit of hits) {
      ...
      if (!best || hit.t < best.t - EPS) {
        best = { edge: hit.edge, t: hit.t, point };
        ...
        if (candidateHorizontal === preferHorizontalEdge && bestHorizontal !== preferHorizontalEdge) {
          best = { edge: hit.edge, t: hit.t, point };
        }
      }
    }
  ...
  function resamplePolyline(points, spacing = RESAMPLE_SPACING) {
    ...
    if (Math.hypot(last.x - tail.x, last.y - tail.y) > 0.1) {
      output.push({ ...last });
    }
    return output;
  }
  ...
  function applyDeadband(points) {
    ...
    for (let i = 1; i < points.length; i += 1) {
      ...
      if (
        i === points.length - 1 ||
        Math.hypot(point.x - previous.x, point.y - previous.y) >= JITTER_DEADBAND
      ) {
        output.push(point);
      }
    }
  ...
  function normalizePointerPath(rawPoints, startNode) {
    ...
    const raw = [{ ...start }, ...rawPoints.slice(1)];
  ...
  function pushSegment(segments, segment) {
    ...
    if (
      previous &&
      previous.cellX === segment.cellX &&
      previous.cellY === segment.cellY &&
      previous.from === segment.to &&
  ...
  function nearestTailPort(cell, entryPort, point) {
    const entryNode = portToNode(cell.x, cell.y, entryPort);
    const entryPoint = nodePosition(entryNode);
    let best = {
      port: entryPort,
      node: entryNode,
      distance: Math.hypot(entryPoint.x - point.x, entryPoint.y - point.y),
    };

    for (const edge of EDGES) {
      if (edge === entryPort[0]) continue;
      for (let index = 0; index < PORT_COUNTS[edge]; index += 1) {
        const port = `${edge}${index}`;
        const node = portToNode(cell.x, cell.y, port);
  ...
  function compilePolyline(points, startNode, explicitEndNode = null, closeTail = true) {
    ...
    const segments = [];
    let currentPoint = nodePosition(startNode);
    let pointIndex = 1;
    ...
    while (pointIndex < points.length) {
      ...
      if (cellContainsPoint(currentCell, target, -0.0001)) {
        currentPoint = target;
        pointIndex += 1;
        continue;
      }
      ...
      if (!cellInBounds(nextCell)) {
        currentPoint = exit.point;
        break;
      }
      ...
      const remainingX = target.x - exit.point.x;
      const remainingY = target.y - exit.point.y;
      const remainingLength = Math.hypot(remainingX, remainingY);
      if (remainingLength > EPS) {
        currentPoint = {
          x: exit.point.x + (remainingX / remainingLength) * 0.001,
          y: exit.point.y + (remainingY / remainingLength) * 0.001,
        };
      } else {
        currentPoint = exit.point;
        pointIndex += 1;
    ...
    if (explicitEndNode) {
      ...
      if (endPort && endPort[0] !== entryPort[0]) {
        pushSegment(segments, {
          cellX: currentCell.x,
          cellY: currentCell.y,
          from: entryPort,
      ...
      const tail = nearestTailPort(currentCell, entryPort, tailPoint);
      if (tail.port !== entryPort && tail.port[0] !== entryPort[0]) {
        pushSegment(segments, {
          cellX: currentCell.x,
          cellY: currentCell.y,
          from: entryPort,
  ...
  function lineSegments(startNode, endNode) {
    ...
    return compilePolyline(
      [nodePosition(startNode), nodePosition(endNode)],
      startNode,
      endNode,
      false,
    );
  ...
  function bezierSegments(nodes) {
    const p = nodes.map(nodePosition);
    const points = [p[0]];
    for (let index = 1; index <= 160; index += 1) {
      points.push(cubicPoint(p[0], p[1], p[2], p[3], index / 160));
    }
  ...
  function ellipseSegments(aNode, bNode) {
    ...
    const startNode = nearestNode(center.x + rx, center.y);
    const start = nodePosition(startNode);
    const points = [start];
    ...
    points.push(start);
    return compilePolyline(points, startNode, startNode, false);
  }

  function semanticCodepoint(segment) {
    return STRAIGHT_CODEPOINT_BY_PAIR[`${segment.from}>${segment.to}`] ?? null;
  }
  ...
  function render() {
    ...
    if (previewSegments.length > 0) {
      allPaths.push({ tool: 'preview', segments: previewSegments });
    }

    const cellMap = buildCellMap(allPaths);
    ...
    if (hoverNode) {
      const p = nodePosition(hoverNode);
      overlayCtx.fillStyle = '#d43b2f';
      overlayCtx.beginPath();
      overlayCtx.arc(p.x, p.y, 3.5, 0, Math.PI * 2);
      overlayCtx.fill();
    }
  ...
  function toolInstruction(tool) {
    if (tool === 'freehand') return 'Freehand: draw naturally; input is bridged, resampled, smoothed, then compiled to exact nodes.';
    if (tool === 'line') return 'Line: drag from one node to another.';
    if (tool === 'bezier') return 'Bezier: click four nodes — start, control 1, control 2, end.';
    return 'Ellipse: drag between two nodes to define its bounds.';
  }
  ...
  overlayCanvas.addEventListener('pointerdown', (event) => {
    ...
    if (currentTool === 'bezier') {
      ...
      if (bezierNodes.length === 4) {
        ...
        setStatus(
          commitSegments('bezier', segments)
            ? `Bezier committed: ${segments.length} exact node-to-node segments.`
            : 'Bezier could not form a legal node path.',
        );
        bezierNodes = [];
      } else {
  ...
  overlayCanvas.addEventListener('pointermove', (event) => {
    ...
    if (currentTool === 'freehand') {
      ...
      for (const sampleEvent of coalesced) {
        pushRawPoint(activeGesture.rawPoints, canvasPoint(sampleEvent));
      }
      pushRawPoint(activeGesture.rawPoints, point);

      const normalized = normalizePointerPath(activeGesture.rawPoints, activeGesture.startNode);
      previewSegments = compilePolyline(normalized, activeGesture.startNode, null, true);
    } else if (currentTool === 'line') {
      previewSegments = lineSegments(activeGesture.startNode, hoverNode);
  ...
  function finishPointerGesture(event) {
    ...
    if (currentTool === 'freehand') {
      ...
      pushRawPoint(activeGesture.rawPoints, point);
      const normalized = normalizePointerPath(activeGesture.rawPoints, activeGesture.startNode);
      segments = compilePolyline(normalized, activeGesture.startNode, null, true);
    }

    const ok = commitSegments(currentTool, segments, currentTool === 'ellipse');
    setStatus(
      ok
        ? `${currentTool} committed: ${segments.length} exact node-to-node segments.`
        : `${currentTool} ended without a legal node path.`,
    );
    previewSegments = [];
    activeGesture = null;
    render();
